...
struct PresetSettings {
  ...
  uint32_t soundIndex; // Which sound file to use
};
...
void setup() {
  ...
  // Initialize SPIFFS for preset storage
...
void initPresets() {
  for (int i = 0; i < PRESET_COUNT; i++) {
    ...
    systemSettings.presets[i].soundIndex = i;
  }
...
// ============================================================================
void savePresetsToFlash() {
  File file = SPIFFS.open("/presets.bin", "w");
  if (!file) {
  ...
  file.close();
  Serial.println("[OK] Presets saved to flash");
}

void loadPresetsFromFlash() {
  if (!SPIFFS.exists("/presets.bin")) {
    Serial.println("[INFO] No presets file found, using defaults");
    savePresetsToFlash();
    return;
  }
  
  File file = SPIFFS.open("/presets.bin", "r");
  if (!file) {
    Serial.println("[ERROR] Failed to open presets file for reading");
    return;
  }
  
  file.read((uint8_t *)&systemSettings, sizeof(SystemSettings));
  file.close();
  Serial.println("[OK] Presets loaded from flash");
}
...
void drawPresetMenu() {
  ...
  // Display all presets with selection indicator
  int startIdx = max(0, (int)systemSettings.currentPreset - 4);
  int endIdx = min(PRESET_COUNT, startIdx + 9);
  
  for (int i = startIdx; i < endIdx; i++) {
    int displayIdx = i - startIdx;
    int yPos = 25 + (displayIdx * 10);
...
void drawEditNameMenu() {
  ...
  tft.setTextColor(TFT_WHITE);
  tft.setCursor(5, 40);
  tft.println("Char Pos:");
  tft.setCursor(5, 50);
  
  // Display name with cursor
  for (int i = 0; i < MAX_PRESET_NAME_LENGTH - 1; i++) {
    if (systemSettings.presets[systemSettings.currentPreset].name[i] == '\0') {
      if (i == nameEditIndex) {
    ...
    tft.print(systemSettings.presets[systemSettings.currentPreset].name[i]);
  }
  ...
  tft.print("Pos: ");
  tft.setTextColor(TFT_YELLOW);
  tft.println(nameEditIndex + 1);
  
  tft.setTextColor(TFT_YELLOW);
  tft.setCursor(5, 80);
  tft.println("Rotate: Char");
  tft.setCursor(5, 90);
  tft.println("Hold Press: Next Pos");
  ...
  tft.setCursor(5, 120);
  tft.print("Avail chars: A-Z, a-z, 0-9, -, _");
}
...
void handleEncoderInput() {
  ...
  if (digitalRead(ENCODER_SW) == LOW && !encoderPressed) {
    ...
    switch (currentMenu) {
      ...
      case 5: // Edit Decay - save and go back
        ...
        savePresetsToFlash();
        break;
      case 6: // Edit Name - toggle character position or save
        if (nameEditIndex < strlen(systemSettings.presets[systemSettings.currentPreset].name)) {
          nameEditIndex++;
          lastEncoderValue = 0;
          encoderValue = 0;
        } else {
  ...
  if (encoderValue != lastEncoderValue) {
    ...
    switch (currentMenu) {
      case 0: // Main menu - not used, just navigate with button
        break;
      case 1: // Preset selection
        ...
        break;
      case 2: // Settings - cycle through settings with encoder
      ...
      case 5: // Edit Decay (1-100)
        ...
        break;
      case 6: // Edit Name - cycle through available characters
        if (nameEditIndex < strlen(systemSettings.presets[systemSettings.currentPreset].name)) {
          int currentCharIdx = 0;
          for (int i = 0; i < charMapSize; i++) {
            if (charMap[i] == systemSettings.presets[systemSettings.currentPreset].name[nameEditIndex]) {
              currentCharIdx = i;
              break;
            }
          }
          currentCharIdx = (currentCharIdx + delta + charMapSize) % charMapSize;
          systemSettings.presets[systemSettings.currentPreset].name[nameEditIndex] = charMap[currentCharIdx];
        }
        break;
    }
  }
...
void processBluetoothCommand(const char *cmd) {
  ...
  if (strncmp(cmd, "PRESET:", 7) == 0) {
    ...
    SerialBT.println("Sabilu Dzikri v1.0");
  } else {
